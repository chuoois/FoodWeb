...
const addToCart = async (req, res) => {
  try {
    ...
    if (food.shop_id.toString() !== shop_id.toString()) {
      return res
        .status(400)
        .json({ message: "Food does not belong to the specified shop" });
    }
    ...
    // ✅ Giờ đây mỗi shop có giỏ riêng, không chặn khác shop
    ...
    if (existingItem) {
      ...
      return res
        .status(200)
        .json({ message: "Updated cart item", item: existingItem });
    }
    ...
    if (error.code === 11000) {
      return res
        .status(409)
        .json({ message: "Cart item conflict, please retry" });
    }
...
const getCart = async (req, res) => {
  try {
    const accountId = req.user.accountId;
    const { shop_id } = req.query; // 🔹 nhận shop_id từ query
    const user_id = await getUserObjectId(accountId);

    const filter = { user_id, status: "ACTIVE" };
    if (shop_id) filter.shop_id = shop_id; // 🔹 nếu có shop_id thì chỉ lấy giỏ của shop đó

    let items = await CartItem.find(filter)
      .populate("food_id")
      .populate("shop_id", "name");

    if (!items.length)
      return res.json({ shop: null, items: [], removed_item_ids: [] });
    ...
    return res.json({
      shop,
      items: resultItems,
      removed_item_ids: unavailableIds,
    });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
...
const updateCartItem = async (req, res) => {
  try {
    ...
    if (!item || item.user_id.toString() !== user_id.toString())
      return res.status(404).json({ message: "Cart item not found" });

    if (item.status !== "ACTIVE")
      return res
        .status(400)
        .json({ message: "Cannot update non-active cart item" });
    ...
    if (!item.food_id || !item.food_id.is_available) {
      ...
      return res
        .status(400)
        .json({
          message: "Food is no longer available and has been removed from cart",
        });
    }
